...
namespace AKTTool.Repository
{
  public class FluentQuery<TEntity> : IFluentQuery<TEntity> where TEntity : EntityBase, new()
  {
    ...
    public FluentQuery(RepositoryBase<TEntity> repositoryBase)
    {
      _repositoryBase = repositoryBase;
      _query = repositoryBase.GetQueryable();
    }

    public FluentQuery(RepositoryBase<TEntity> repositoryBase, IQueryObject<TEntity> queryObject)
        : this(repositoryBase)
    {
      _query = _query.Where(queryObject.Query());
    }

    public FluentQuery(RepositoryBase<TEntity> repositoryBase, Expression<Func<TEntity, bool>> filter)
        : this(repositoryBase)
    {
